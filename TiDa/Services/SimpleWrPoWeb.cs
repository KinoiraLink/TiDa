    ...
    public  class SimpleWrPoWeb : IDataWeb<SimpleWrPo>
    {
        ...
        public async Task<IList<SimpleWrPo>> UploadAsync(IList<SimpleWrPo> simpleWrPos)
        {
            ...
            try
            {
                ...
                response = await httpClient.PostAsync("http://121.37.91.77:3000/upload/simpleget", content);
                json = null;
            ...
            if (romoteInsert.Count > 0)
            {
                try
                {
                    ...
                    StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
                    response = await httpClient.PostAsync("http://121.37.91.77:3000/upload/simplesave", content);
                    json = await response.Content.ReadAsStringAsync();
            ...
            if (romoteUpdate.Count > 0)
            {
                try
                {
                    ...
                    StringContent content = new StringContent(json, Encoding.UTF8, "application/json");
                    response = await httpClient.PostAsync("http://121.37.91.77:3000/upload/simpleupdate", content);
                    json = await response.Content.ReadAsStringAsync();
                    var simpleWrPoTaskList = JsonConvert.DeserializeObject<IList<SimpleWrPo>>(json);
        ...
        public async Task<int> CountAsync(SimpleWrPo item)
        {
            ...
            json = null;
            json = await response.Content.ReadAsStringAsync();
            var i = JsonConvert.DeserializeObject<IList<SimpleWrPo>>(json);
            return i.Count;
        }
    }
}
